...
class CubedSphere(object):
    def __init__(self, sphere_coords):
        ...
        # every azimuth angle
        num_elevation_measurements = {}
        elevation_indices = {}
        for key in sphere_coords:
            num_elevation_measurements[key] = sphere_coords[key].shape[0]
            elevation_indices[key] = list(range(num_elevation_measurements[key]))
        ...
        for azimuth_index, azimuth in enumerate(sphere_coords.keys()):
            ...
            self.sphere_coords += list(zip(elevation.tolist(), [azimuth] * num_elevation_measurements[original_azimuth]))
            self.indices += list(zip(elevation_indices[original_azimuth], [azimuth_index] * num_elevation_measurements[original_azimuth]))
