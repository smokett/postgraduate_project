...
class TrainValidHRTFDataset(Dataset):
    ...
    def __getitem__(self, batch_index: int) -> [torch.Tensor, torch.Tensor]:
        ...
        #hrtf = hrtf[0]
        hrtf = torch.permute(hrtf, (0,3,1,2))
        if self.transform is not None:
            for i in range(hrtf.shape[0]):
                hrtf[i] = self.transform(hrtf[i])
        hrtf = torch.permute(hrtf, (0,2,3,1))
        hrtf = torch.permute(hrtf,(1,2,3,0))
        hrtf = torch.reshape(hrtf,(hrtf.shape[0], hrtf.shape[1], hrtf.shape[2]*hrtf.shape[3]))
