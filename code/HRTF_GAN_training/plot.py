...
def plot_magnitude_spectrums(frequencies, magnitudes_real, magnitudes_interpolated, ear, label, path,
...
def plot_SD_node(sr, hr, epoch, path, file):
    # batch, frequency, panel, width, height
    x = []
    y = []
    diff = []
    diff_left = []

    sr_selected = sr.detach().cpu()[0, :, :, :, :]
    hr_selected = hr.detach().cpu()[0, :, :, :, :]
    for p in range(5):
        for w in range(16):
            for h in range(16):
                x_y = convert_cube_indices_to_spherical(p, w, h, 16)
                x.append((x_y[1]/np.pi) * 180)
                y.append((x_y[0]/np.pi) * 180)
                sum_on_band = 0.0
                for j in range(10,128):
                    sum_on_band += (20.0 * np.log10(abs(hr_selected[j,p,w,h]) / abs(sr_selected[j,p,w,h])))**2
                sum_on_band /= 118
                sum_on_band_1 = 0.0
                for j in range(138,256):
                    sum_on_band_1 += (20.0 * np.log10(abs(hr_selected[j,p,w,h]) / abs(sr_selected[j,p,w,h])))**2
                sum_on_band_1 /= 118
                sum_on_band = np.sqrt(sum_on_band.numpy())
                sum_on_band_1 = np.sqrt(sum_on_band_1.numpy())
                diff.append((sum_on_band+sum_on_band_1)/2.0)
                diff_left.append(sum_on_band)
    
    file.write(str(round(sum(diff)/1280,5))+"\n")
    plt.scatter(x,y,c=diff, cmap="coolwarm",vmin=0,vmax=20)
    plt.colorbar()
    plt.title("SD per node -- HR vs SR "+"avg: "+str(round(sum(diff_left)/1280,5)))
    plt.xlabel("azimuth")
    plt.ylabel("elevation")
    plt.savefig(f'{path}/{epoch}_SD_node.png', dpi=300)
    plt.close()

def plot_SD_frequency(sr, hr, epoch, path):
    # batch, frequency, panel, width, height
    ...
    for f in range(128):
        ...
        for p in range(5):
            for w in range(16):
                for h in range(16):
                        sum_on_node += (20.0 * np.log10(abs(hr_selected[f,p,w,h]) / abs(sr_selected[f,p,w,h])))**2
        sum_on_node /= 16*16
        diff.append(np.sqrt(sum_on_node))
    
    plt.bar(x,diff,width=50)
    plt.title("SD per frequency -- HR vs SR")
    plt.xlabel("frequency")
    plt.ylabel("SD")
    plt.savefig(f'{path}/{epoch}_SD_frequency.png', dpi=300)
    plt.close()